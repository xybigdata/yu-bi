...
describe('test migrateBoard ', () => {
  ...
  test('should migrate auto board spacing into jsonConfig', () => {
    ...
    expect(basic?.rows?.find(row => row.key === 'initialQuery')?.value).toBe(
      false,
    );
    expect(basic?.rows?.find(row => row.key === 'allowOverlap')?.value).toBe(
      true,
    );
