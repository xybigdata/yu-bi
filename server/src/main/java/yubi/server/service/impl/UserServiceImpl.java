...
import yubi.core.base.exception.ServerException;
...
import yubi.server.common.OAuth2AttributeMapping;
...
import static yubi.core.common.Application.getProperty;
...
public class UserServiceImpl extends BaseService implements UserService {
    ...
    @Value("${yubi.user.active.send-mail:false}")
    ...
    public User externalRegist(OAuth2AuthenticationToken oauthAuthToken) throws ServerException {
        ...
        user.setCreateBy(user.getId());
        user.setCreateTime(new Date());
        user.setName(OAuth2AttributeMapping.readString(attributeDocument, nameMapping));
        ...
        if (emailMapping != null) {
            user.setEmail(OAuth2AttributeMapping.readString(attributeDocument, emailMapping));
        }
        if (avatarMapping != null) {
            user.setAvatar(OAuth2AttributeMapping.readString(attributeDocument, avatarMapping));
        }
    ...
    public User addUserToOrg(UserAddParam userAddParam, String orgId) throws MessagingException, UnsupportedEncodingException {
        ...
        if (StringUtils.isBlank(userAddParam.getEmail())) {
            ...
            userAddParam.setEmail(DigestUtils.md5Hex(str) + "@yubi.generate");
        }
