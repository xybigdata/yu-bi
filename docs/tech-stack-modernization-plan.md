...
当前正在推进的是“控制器配置态基础 setter 值类型边界收口”：

- `SingleTimeSet / TextSetForm / RadioStyleSet / SqlOperatorSet` 仍保留较宽的 `value` / `onChange` / `FormItemProps<any>`
- 本轮只按当前真实使用形态收紧配置态基础 setter 的值与回调类型
- 不改表单交互，不改运行时行为
