...
截至 2026-06-12，工作区不是全干净状态，除文档外还存在一组未提交的前端依赖治理改动：

- `frontend/package.json`
- `frontend/package-lock.json`

这组改动当前表现为：移除前端未直接使用的 `@antv/g2` 直接依赖。它应作为独立专题处理，不与本文档整理混在同一提交里。
...
- `@antv/g2` 的直接依赖清理目前在工作区中，尚未并入已完成项
