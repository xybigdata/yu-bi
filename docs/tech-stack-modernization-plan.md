...
| P2-E-Next | 前端构建产物治理               | 中   | 可推进；JS gzip 超限已清零，构建报告已补 `category` 分类，后续优先分析 raw 维度的 `monacoEditor`、`antdDesign` 和地图资源 |
...
- 构建报告新增 `category` 分类输出，区分 `vendor`、`runtime`、`task`、`geo`、`asset` 等类型，便于继续治理 raw 超限项
